...
OUTPUT_DIR = "../out/spring_quarter/time_slice_test"
...
def main():
    ...
    host_types = ['global']
    deq_costs = [0.0]
    queue_policies = ['time_slice_test']
    ...
    slices = [5 * i for i in range(13,21)]
    for i in slices:
        temp_conf = copy.deepcopy(default_json)
        temp_conf[0]["time_slice"] = i
    ...
    slices = [10 * i for i in range(6,15)]
    for i in slices:
        temp_conf = copy.deepcopy(default_json)
        temp_conf[0]["time_slice"] = i
        config_jsons.append(temp_conf)
